...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* constHelp(std::vector<Point<Dim>>& points, int dimen, unsigned left, unsigned right);
    Point<Dim>& qSelect(vector<Point<Dim>>& list, int dimen, unsigned left, unsigned right, unsigned i);
    void copy(KDTreeNode* subroot, KDTreeNode* root2);
